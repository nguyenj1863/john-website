...
export default function AlbumCollage() {
    ...
    return (
        <div onClick={handleClickOutside}>
            <h3 className="text-2xl font-body font-bold text-[var(--primary)] mb-4 tracking-tighter neon-glow">Favourite Albums</h3>
                    ...
                    >
                        ...
                        >
                            <img
                                ...
                                className="w-full h-full object-cover transition-all border border-[var(--primary)] rounded-xl"
                            />
                        </div>

                        <div className={`absolute sm:flex flex-col -top-2 left-1/2 transform -translate-x-1/2 -translate-y-full bg-[var(--background)] p-2 sm:opacity-0 sm:group-hover:opacity-100 transition-all z-10 text-xs border border-primary whitespace-nowrap w-auto min-w-[100px] lg:min-w-[120px] max-w-[300px] pointer-events-none rounded-xl]
                            ${activeAlbum === index ? 'flex opacity-100' : 'hidden opacity-0'}`}
                        >
                            <p className="font-bold font-body text-center text-wrap leading-tight tracking-tighter neon-glow" title={album.title}>{album.title}</p>
